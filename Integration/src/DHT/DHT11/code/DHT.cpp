...
unsigned char DHT::expectPulse(bool level)
{
    ...
    while (digitalRead(data_pin) == level) { count++; }
    return count;
}
/**************************-**********************************
*                              *
  ...
  for (int i=0; i<32; i++)
  {
   ...
   unsigned char highCycles = Pulse[2*i+1];
    ...
    else if (i<16 )
    {
       ...
       if (highCycles > lowCycles)  RH_dec |= 1;      // High cycles are greater than 50us low cycle count, must be a 1.
    }
     else if (i<24)
    {
                                   temperature_int<<=1;
...
int  DHT::temperature_cmp(DHT passed ,int error=10)
{
  ...
  if ((temperature>passed.get_temperature())>temperature_error) return 1;
  if ((temperature>passed.get_temperature())<temperature_error) return -1;
  return 0;
...
int  DHT::RH_cmp(DHT passed ,int error)
{
  ...
  if ((RH>passed.get_RH())>RH_error) return 1;
  if ((RH>passed.get_RH())<RH_error) return -1;
  return 0;
}
