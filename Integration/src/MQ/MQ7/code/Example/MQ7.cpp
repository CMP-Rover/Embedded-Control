...
void MQ7::GetGasConcentration(int &analogReading, float &mappedVoltage, float &mappedPPM, bool mapToPPM)
{
  analogReading = analogRead(analog_pin);
  if (analogReading < MQ7SensitivityThresholds[detectionThreshold])
  {
    analogReading = 0;
    gasDetected = false;
  }
  ...
  if (mapToPPM)
  {
    mappedVoltage = mapfloat(analogReading, 0, 1023, 2.5, 4.3); // Map analog to voltage
...
void MQ7::PrintMQ7Data(bool plotter)
{
  int analog;
  float voltage, PPM;
  GetGasConcentration(analog, voltage, PPM, true);
